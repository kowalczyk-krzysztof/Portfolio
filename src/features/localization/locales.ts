...
export type Locale = {
  ...
  readonly homeTechnologies: string;
  readonly homeHobbies: string;
};
...
export const locale_PL: Locale = {
  ...
  homeTechnologies: `Technologie, których używam:`,
  homeHobbies: `W wolnym czasie lubię pracować nad sobą. Uważam, że zdrowy tryb życia ma ogromny wpływ na produktywność, więc staram się żyć aktywnie. Moim hobby jest uczenie się języków - gdy poznasz obcy język, otwiera się przed tobą zupełnie nowy świat, co poszerza twoje horyzonty. Obecnie posługuje się biegle językiem angielskim i polskim oraz japońskim w stopniu zaawansowanym.`,
};
...
export const locale_JP: Locale = {
  ...
  homeDescription: `Krzysztofです｡ フルスタック開発者｡ なぜソフトウェア開発しますか？ 私の目指しはお役に立てる高品質のソフトウェアをお客様に届けることです｡ オンラインで多くの時間を過ごすのでインターネットユーザーの欲しい物をよく分かります｡ 効率とユーザーエクスペリエンスのバランスを見つけるのは本当に難しいことです｡ でも私はそれをできると信じています｡ 問題があるなら直ぐに解決します｡`,
  homeTechnologies: `下の技術を使用しています:`,
  homeHobbies: `空き時間に自分磨きをしています｡ 生産性と健康状態の関連があると思うので健康な生活を送っています｡ 外国語を学ぶことが大好きです｡ 可能性が広がり自分を成長させたりできます｡ 現在英語とポーランド語が話せます｡ 日本語がペラペラになるように頑張っています｡`,
};
